...
export interface CoinReport {
  ...
  volume24h?: number;
  aiAnalysis: { summary: string; pumpScore?: number; reasons: string[] };
  whale: { totalUsd: number; count: number; text: string };
  risk: { score?: number; flags: string[]; text: string };
...
export function getCoinReportData(query: string, snap: NoctrunSnapshot): CoinReport {
  ...
  return {
    ...
    risk: {
      ...
      flags: risk?.flags ?? [],
      text: risk
        ? `Rugpull score ${risk.score}/100.`
        : "Tidak ada sinyal rugpull dari data DEX saat ini — bukan jaminan aman, tetap DYOR.",
    },
    momentum: {
