...
// Every function below is a pure re-read of data Noctrun already fetches in
 ...
 * field Noctrun already has on hand. Excludes BTC itself and major
 * stablecoins. Markets are assumed pre-sorted by market cap desc (CoinGecko
...
export function computeAltseasonIndex(markets: CoinMarket[]): AltseasonReading | undefined {
  ...
  const sample = markets
    .filter((m) => !STABLE_SYMBOLS.has(m.symbol.toLowerCase()) && m.symbol.toLowerCase() !== "btc")
...
/**
 * Biggest real 24h decliners among reasonably liquid names. Deliberately
 * NOT called a "prediction" anywhere in the UI copy — it's a read of what
 * already moved, the honest substitute for a fabricated dump-probability
 * score the scoring engine doesn't actually compute.
 */
export function computeTopDecliners(markets: CoinMarket[], limit = 5): CoinMarket[] {
  return markets
    .filter((m) => !STABLE_SYMBOLS.has(m.symbol.toLowerCase()))
...
/**
 * Tokens whose rugpull flags specifically describe a wash-trading-style
 * pattern (volume dwarfing the pool, or a brand-new pool already carrying
 * heavy flow) rather than just generic thin liquidity. This is the
 * narrowest, most defensible reading of "manipulation" the current scoring
 * engine supports — it re-labels existing flags, it doesn't invent new ones.
 */
export function computeManipulationAlerts(rugpullRisks: RugpullRisk[], limit = 5): RugpullRisk[] {
...
/** Short, terminal-style status line for the mobile header. */
export function computeSystemTagline(fngValue: number | undefined, riskCount: number, pumpCount: number): string {
  ...
  if (riskCount >= 8) return "Elevated Rugpull Signals";
