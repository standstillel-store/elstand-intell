...
import { PaperTraderTeaser } from "@/components/paper-trader/PaperTraderTeaser";
...
export default async function Home() {
  ...
  const [wallet, stats, newSignals] = await Promise.all([getWallet(), getStatistics(), listSignals({ status: "new", limit: 50 })]);
  const paperWallet = wallet ?? getDefaultWallet();
  const paperStats = stats ?? getDefaultStatistics();
  ...
  return (
    <main className="min-h-screen">
      ...
      <div className="hidden pb-20 lg:block lg:pl-60">
        <Sidebar />
        <TopBar
        ...
        <div className="mx-auto max-w-7xl px-4 py-6">
          ...
          <div className="mb-6">
            <PaperTraderTeaser wallet={paperWallet} stats={paperStats} newSignalCount={newSignals.length} />
          </div>
