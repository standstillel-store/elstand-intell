...
async function fetchChainWhales(chainKey: string): Promise<any[]> {
...
export async function GET() {
  ...
  try {
    ...
    const allTransfers = chainResults.flat();
    ...
    if (whales.length === 0) {
      return Response.json({
        transactions: DEMO_WHALES,
        isDemo: true,
        message: `Nggak ada native transfer >= $${WHALE_USD_THRESHOLD.toLocaleString()} dalam ~${BLOCK_LOOKBACK.toLocaleString()} block terakhir. Showing demo data.`,
        chains: CHAIN_LABELS,
      });
    }
