...
// Noctrun's chat dock used to proxy every question to the OpenAI API, which
// costs real money per request. It now runs entirely on Nocturn's own
// rule-based Intelligence Engine (lib/analysis.ts): live data in, structured
// signal interpretation out — no LLM call, no API key, no cost, ever.
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[Noctrun] chat engine error:", err);
