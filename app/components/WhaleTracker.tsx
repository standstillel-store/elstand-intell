...
export default function WhaleTracker() {
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 md:col-span-2">
      <h2 className="text-xs text-slate-400 uppercase mb-4 font-mono">🐳 Whale Tracker (Alchemy)</h2>
          ...
          <div key={w.hash} className="border border-slate-600 rounded p-3 bg-slate-700/50">
            <div className="flex justify-between mb-2">
              ...
              <div className="text-right">
                ...
                <p className="text-green-400 text-xs font-mono">${w.usdValue.toLocaleString()}</p>
              </div>
            </div>
            <div className="flex justify-between text-xs">
              <a
                href={`${EXPLORERS[w.chain] || EXPLORERS.Ethereum}${w.hash}`}
                target="_blank"
