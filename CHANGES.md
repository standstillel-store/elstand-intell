...
## V2.9 — Phase 3: Authentication, Wallet Connect, AI Energy (production-ready pass)

Brief-nya "ELSTAND INTEL — PHASE 3 (AUTHENTICATION & USER SYSTEM)": bangun sistem user penuh di atas dashboard yang sebelumnya single-tenant (satu deployment = satu trader, tanpa konsep user_id sama sekali di `ai_signals`/`ai_journal`/`paper_wallet`/`bn_*`). Round ini genuinely per-user: Google login, wallet EVM, AI Energy — tanpa nyentuh UI utama atau logic Paper Trading yang udah stabil.

**Authentication & Session** — "Continue with Google" dan cookie-based session sebenarnya sudah ada duluan (`lib/auth/client.ts`/`server.ts` pakai `@supabase/ssr`, bukan localStorage — ini yang bikin session persist lewat refresh/tutup tab/restart browser dari awal, `middleware.ts` yang refresh token-nya tiap request). Yang ditambah round ini: `app/auth/callback/route.ts` sekarang provision baris `users`/`profiles`/`ai_token`/`user_settings` (`lib/auth/profile.ts`, upsert — sekali jalan baik buat "akun baru" maupun "update last_login_at"), catat device + activity log, terus lempar ke `/auth/success` (animasi checkmark singkat, respect `prefers-reduced-motion`) sebelum masuk `/dashboard` — gak pernah balik ke landing page. `middleware.ts` sekarang juga redirect user yang udah login menjauh dari `/` (dulu cuma `/login`).

**Wallet Connect** — Reown AppKit + wagmi (`lib/web3/config.ts`), MetaMask/Rabby/OKX ter-auto-detect (EIP-6963), Coinbase Wallet + WalletConnect punya connector sendiri. Sempat CRASH karena `WagmiProvider`/AppKit belum dipasang di manapun — fixed di `app/layout.tsx` (root layout, pakai `headers()` buat `cookieToInitialState` biar wagmi hydrate tanpa mismatch — trade-off yang didisclose di komentar file itu: landing page jadi ikut dynamic rendering, gak bisa static lagi). Connect wallet WAJIB lewat sign-to-verify (`lib/wallet/verify.ts`, `lib/wallet/message.ts`) sebelum baris `wallets` ditulis — server recover address dari signature pakai `viem`'s `recoverMessageAddress` (gak butuh RPC). Ini skema custom ringan, BUKAN SIWE/EIP-4361 penuh — didokumentasikan jujur di kode. Gak pernah nyimpen/minta private key atau mnemonic, titik.

**AI Energy** — `lib/energy.ts`: 10/hari, reset dicek lazy tiap balance dibaca/dipakai (bukan cron), tiap transaksi kecatat ke `ai_token_transactions`. Beneran wired ke 5 route AI (`lib/energyGate.ts`): chat (1), generate 1 signal (1), full watchlist scan (3), chart analysis (1), token analyzer (1) — bukan cuma angka nganggur di database. Spend pakai optimistic-concurrency guard (`.eq("balance", ...)` di update) biar dua request barengan gak double-spend diam-diam — bukan atomic transaction beneran, cukup buat traffic app ini, didokumentasikan di komentar kalau butuh lebih ketat nanti.

**Settings & TopNav** — Section baru: Account (avatar/username/email/member since/last login — read-only, sumber dari Google), AI Energy (balance + reset countdown + riwayat transaksi), Wallet (connect/verify/disconnect + list). Security diperluas: riwayat device beneran (bukan cuma UA browser sekarang), Activity Log (20 event terakhir), Logout (device ini doang, scope `local`) DAN Logout All Devices (scope `global`, sudah ada). Danger Zone tambah Delete Account (double-confirm pattern yang sama kayak Reset Paper Trading, panggil `supabase.auth.admin.deleteUser()` via service-role key — cascade ke semua tabel Phase 3 lewat `ON DELETE CASCADE`). TopNav dirombak: avatar Google asli, username, email, status wallet (address terpotong + dot hijau kalau connected), AI Energy balance, Logout — semua dari satu endpoint gabungan `/api/account/me` biar gak 4x fetch tiap page load. "Notification" di brief = `AlertsBell` yang emang udah ada di situ dari sebelumnya, gak disentuh.

**Database** — Section baru di `supabase/schema.sql` (sama file, tinggal dijalankan ulang — semua statement idempotent): `users`, `profiles`, `wallets`, `devices`, `ai_token` + `ai_token_transactions`, `user_settings`, `activity_log`, plus tabel payment-prep (`payment_history`, `topup_history`, `wallet_topup`, `payment_provider`, `transaction_status` — schema doang, belum ada logic pembayaran apapun, sesuai brief). Beda dari tabel single-tenant di atasnya (RLS enabled tapi nol policy publik, service-role only), semua tabel Phase 3 pakai RLS per-user beneran (`auth.uid() = user_id`).

**PENTING — `paper_wallet` SENGAJA gak dimigrasi round ini.** Brief minta `paper_wallet` ikut connect ke `user_id`, tapi itu singleton yang dipakai di seluruh Paper Trader engine (`ai_signals`/`ai_journal`/`ai_statistics` ikut, gak bisa dipisah). Migrasi setengah-setengah (wallet per-user, riwayat trade masih shared) lebih buruk dari kondisi sekarang. **Paper Wallet Multi-User akan dikerjakan di Phase 3.5** — proyek sendiri yang nyentuh seluruh trading engine, bukan side-effect dari migrasi auth. Logic Paper Trading tidak diubah sama sekali round ini.

**Bug kecil yang kena fix pas final check:** `AccountSection` awalnya pakai `toLocaleDateString`/`toLocaleString` tanpa `timeZone` eksplisit — karena dia ke-render di client tree (`SettingsView` adalah Client Component), ini bisa mismatch antara SSR (timezone server) dan hydration (timezone browser). Di-pin ke `timeZone: "UTC"` di kedua fungsi. Juga: `createSupabaseBrowserClient()` throw (bukan return null) kalau env var belum diset — satu tempat (TopNav logout) sempat gak punya `catch`, cuma `finally`, jadi error bakal ke-propagate uncaught; sudah dibungkus try/catch yang benar.

Belum digarap / batasan yang jujur diakui: (1) "Connected Devices" itu riwayat login, bukan live session list — Supabase gak expose API client-safe buat revoke SATU device tertentu, jadi kontrolnya cuma 3 scope asli (`local`/`others`/`global`), didokumentasikan di komentar `schema.sql`. (2) Sign-to-verify wallet itu skema custom, bukan SIWE penuh. (3) Gak bisa jalanin `npm install`/`next build` di sandbox ini (no network access — sama kayak kondisi project ini pertama kali ditulis, lihat komentar di `next.config.js`), jadi verifikasi TypeScript/build beneran perlu dijalanin sendiri di environment kamu. (4) Root layout sekarang dynamic (gara-gara `headers()` buat wagmi SSR) — landing page kehilangan static generation, trade-off yang didisclose, bisa direvisit kalau TTFB landing page jadi masalah beneran.
